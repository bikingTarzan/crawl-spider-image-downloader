...
class downloader(object):
    ...
    def load_config(self):
        self.r = redis.Redis(host=environ.get('redis_host'), port=environ.get('redis_port'))
        self.images_save_path = environ.get('images_path')
        self.client = KafkaClient(hosts=environ.get('kafka_host'))
        self.thumb_folder = environ.get('thumb_folder')
        self.thumb_width = environ.get('thumb_width')
        self.console = str(environ.get('console')) == 'True'
        self.allow_suffix = ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.ico', '.icon', '.bmp']
        self.download_time_out = int(environ.get('download_time_out'))
    ...
    def get_consumer(self, topic):
        if topic not in self.consumers:
            download_img_topic = self.client.topics[str(environ.get(topic))]
            consumer = download_img_topic.get_simple_consumer(
                consumer_group=str(environ.get('kafka_consumer_group')),
                consumer_id=str(environ.get('kafka_consumer_id')),
